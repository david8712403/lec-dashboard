        ...
        case 'STUDENT': {
          ...
          if (isNew) {
            await onCreateStudent?.({
              ...
              status: editingData.status ?? StudentStatus.PENDING,
        ...
        case 'SLOT': {
          ...
          if (isNew) {
            await onCreateSlot?.({
              ...
              time_slot: editingData.time_slot,
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm h-full flex flex-col border border-slate-200 overflow-hidden relative">
      ...
      {modalType === 'ASSESSMENT' && (
        <Modal title={editingData?.id ? "編輯檢測報告" : "新增檢測報告"} onClose={closeModal} onSubmit={handleSave} onDelete={editingData?.id ? handleDelete : undefined}>
          <div className="grid grid-cols-2 gap-4">
             <div className="col-span-2">
